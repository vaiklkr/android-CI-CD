...
import '../../../api/valid mpin menu/model/valid_mpin_model.dart';
import 'newmpin.dart';

class ChangeMenuMPinScreen extends StatefulWidget {
  const ChangeMenuMPinScreen({super.key, required this.currentMpin});
  // final TextfiledType textfiledType;
  final String currentMpin;

  @override
  State<ChangeMenuMPinScreen> createState() => _ChangeMenuMPinScreenState();
}

class _ChangeMenuMPinScreenState extends State<ChangeMenuMPinScreen> {
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            AppText(
              text: getLabel(enter_current_m_pin),
  ...
  dynamic onVerfifyMpin() async {
    try {
      Loader().showLoader(context);
      if (!mounted) return;
      final ValidMpinModel isCurrentMpin = await _repository.onValidMipin(
        firstPinController.text,
      );
      Loader().hideLoader(context);
      if (isCurrentMpin.success == true) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) {
              return const NewMenuMPinScreen();
            },
          ),
        );
